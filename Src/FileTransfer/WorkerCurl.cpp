...
int CWorkerCurl::CurlSocketCallback(CURL *easy, curl_socket_t s, int what, void *userp, void *socketp)
{
    ...
    if(!self) return 0;
    return self->OnSocket(what, s, socketp);
}
...
int CWorkerCurl::OnSocket(int what, curl_socket_t s, void *socketp)
{
    //qDebug(log) << Q_FUNC_INFO;
    if(CURL_POLL_REMOVE == what) {
        ...
        curl_multi_assign(m_pMulti, s, nullptr);
    } else {
        AddSocketNotifiers(what, s, socketp);
    }
    return 0;
}
...
int CWorkerCurl::AddSocketNotifiers(int what, curl_socket_t s, void *socketp)
{
    auto si = static_cast<SocketInfo*>(socketp);
    if(!si) {
        //TODO: modify leak!!!
...
{
    qDebug(log) << Q_FUNC_INFO;
    if(!pReq || !pReq->GetHandle()) return;
    auto rc = curl_multi_remove_handle(m_pMulti, pReq->GetHandle());
    if(CURLM_OK != rc)
