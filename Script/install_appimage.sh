...
usage_dev() {
    ...
    echo "In the same directory, the following files must exist:"
...
check_parameters() {
    if [ ! -f *.AppImage ]; then
        usage_dev "Don't find AppImage file(.AppImage)"
    fi
    if [ ! -f *.desktop ]; then
        usage_dev "Don't find desktop file(.desktop)"
    fi
    if [ -z "$(shopt -s nullglob; files=(*.png *.svg); echo ${files[@]})" ]; then
        usage_dev "Don't find icon file(.png or .svg)"
    fi
    if [ -z "$APP_ID" ]; then
        APP_ID=$(basename $(ls *.desktop) .desktop)
    fi
    if [ -z "$APP_ID" ]; then
        usage_long
    fi
